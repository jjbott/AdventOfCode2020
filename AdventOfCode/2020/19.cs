...
namespace AdventOfCode._2020
{
    class _19
    {
        public void RunA()
        {
            ...
            var ruleLines = sections[0].Split("\r\n");
            var messages = sections[1].Split("\r\n");

            var parsed = new Dictionary<int, string>();
            var preparsed = new Dictionary<int, List<List<int>>>();
            foreach(var rl in ruleLines)
            {
                var parts = rl.Split(": ");

                if( parts[1] == "\"a\"")
                {
                    parsed[int.Parse(parts[0])] = "a";
                    continue;
                }
                
                if (parts[1] == "\"b\"")
                {
                    parsed[int.Parse(parts[0])] = "b";
                    continue;
                }

                var ors = parts[1].Split(" | ");
                var rules = new List<List<int>>();
                foreach(var or in ors)
                {
                    var rule = or.Split(" ", StringSplitOptions.RemoveEmptyEntries).Select(x => int.Parse(x)).ToList();
                    rules.Add(rule);
                }
                preparsed[int.Parse(parts[0])] = rules;
            }

            while(preparsed.Any())
            {
                var resolvableRule = preparsed.Where(p => p.Value.SelectMany(r => r).All(r => parsed.ContainsKey(r))).First();
                var rule = string.Join("|", resolvableRule.Value.Select(r => string.Join("", r.Select(ri => "(" + parsed[ri] + ")")) ));
                parsed[resolvableRule.Key] = rule;
                preparsed.Remove(resolvableRule.Key);
            }
            var mainRule = $"^{parsed[0]}$";
            var matchCount = 0;
            foreach(var m in messages)
            {
                if(Regex.IsMatch(m, mainRule))
                {
                    matchCount++;
                }
            }
        ...
        public void RunB()
        {
            ...
            foreach (var rl in ruleLines)
            {
                ...
                if (parts[0] == "8")
                {
                    parts[1] = "42 | 42 42 | 42 42 42 | 42 42 42 42 | 42 42 42 42 42 | 42 42 42 42 42 42";
                }

                if (parts[0] == "11")
                {
                    parts[1] = "42 31 | 42 42 31 31 | 42 42 42 31 31 31 | 42 42 42 42 31 31 31 31 | 42 42 31 31 | 42 42 42 42 42 31 31 31 31 31 | 42 42 42 42 42 31 42 31 31 31 31 31";
                }
        ...
        private string testcase1 = @"0: 4 1 5
1: 2 3 | 3 2
2: 4 4 | 5 5
3: 4 5 | 5 4
4: ""a""
...
aaaabbb";
        ...
        private string input = @"60: 117 7 | 89 13
38: 7 45 | 13 32
77: 7 51 | 13 48
23: 72 13 | 27 7
61: 105 7 | 7 13

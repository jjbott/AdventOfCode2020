...
namespace AdventOfCode._2020
{
    ...
    {
        public string color;
        public Dictionary<string, int> insideBags = new Dictionary<string, int>();

        public Bag(string line)
        {
            var data = line.Split(" bags contain ");
            color = data[0];
            if ( data[1] != "no other bags.")
            {
                var bags = data[1].Split(",");
                foreach(var b in bags)
                {
                    var match = Regex.Match(b, @"([0-9]+) ([ a-z]+) bag");

                    insideBags[match.Groups[2].Value] = int.Parse(match.Groups[1].Value);
                }
    ...
    class _7
    {
        public void RunA()
        {
            ...
            var next = bags.Where(b => b.insideBags.ContainsKey("shiny gold")).ToHashSet();
            ...
            {
                var current = next;
                next = new HashSet<Bag>();

                foreach (var b in current)
                {
                    var nextBags = bags.Where(nb => nb.insideBags.ContainsKey(b.color)).Where(nb => !visited.Contains(nb.color)).ToList();
                    foreach (var nextBag in nextBags)
                    {
                        visited.Add(nextBag.color);
                        next.Add(nextBag);
                    }
                }
        ...
        private int getTotalCount(string color, IDictionary<string, Bag> bags, IDictionary<string, int> countInsideColor)
        {
            if (countInsideColor.ContainsKey(color))
            {
                return countInsideColor[color];
            }

            var bag = bags[color];
            if (!bag.insideBags.Any())
            {
                return 1;
            }

            var count = bag.insideBags.Select(b => getTotalCount(b.Key, bags, countInsideColor) * b.Value).Sum() + 1;
            countInsideColor[color] = count;
            return count;
        }

        public void RunB()
        {
            var bags = input.Split("\r\n").Select(l => new Bag(l)).ToDictionary(b => b.color, b => b);

            var countInsideColor = new Dictionary<string, int>();
            

            // "- 1" to avoid counting the siny gold bag itself
            Console.WriteLine(getTotalCount("shiny gold", bags, countInsideColor) - 1);

        }


        private string input = @"dim silver bags contain 3 posh fuchsia bags.

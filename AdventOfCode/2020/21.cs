    ...
    class _21
    {
        class Food
        {
            ...
            public Food(string line)
            {
                ...
                Allergens = parts[1].Split(")")[0].Split(", ").ToList();
            }
        ...
        public void RunA()
        {
            ...
            var ingredientNotAllergen = new Dictionary<string, HashSet<string>>();

            foreach (var a in allAllergens)
            {
                var f = foods.Where(f => f.Allergens.Contains(a));
                var candidates = f.Skip(1)
                    .Aggregate(
                        new HashSet<string>(f.First().Ingredients),
                        (h, e) => { h.IntersectWith(e.Ingredients); return h; }
                    );
                //var all = f.Aggregate(new HashSet<string>(), (h, e) => { h.UnionWith(e.Ingredients); return h; });

                foreach( var i in allIngredients)
                {
                    if ( !candidates.Contains(i))
                    { 
                        if ( !ingredientNotAllergen.ContainsKey(i) )
                        {
                            ingredientNotAllergen[i] = new HashSet<string>();
                        }

                        ingredientNotAllergen[i].Add(a);
                    }
                }
            }

            var notAllergens = ingredientNotAllergen.Where(kv => kv.Value.Count == allAllergens.Count()).Select(kv => kv.Key);
            Console.WriteLine(foods.Sum(f => notAllergens.Intersect(f.Ingredients).Count()));
        ...
        public void RunB()
        {
            ...
            var notAllergens = ingredientNotAllergen.Where(kv => kv.Value.Count == allAllergens.Count()).Select(kv => kv.Key).ToList();
            foreach(var f in foods)
            {
                foreach(var na in notAllergens)
                {
                    f.Ingredients.Remove(na);
                }
            }

            var ingredientToAllergen = new Dictionary<string, string>();

            while (foods.Any(f => f.Ingredients.Count >= 1))
            {
                foreach (var a in allAllergens)
                {
                    var f = foods.Where(f => f.Allergens.Contains(a));
                    if (f.Any())
                    {
                        ...
                        {
                            if (ingredientToAllergen.ContainsKey(candidates.Single()))
                            {
                                throw new Exception("dang");
                            }

                            ingredientToAllergen[candidates.Single()] = a;

                            foreach (var fi in foods)
                            {
                                fi.Ingredients.Remove(candidates.Single());
                                fi.Allergens.Remove(a);
                            }
                        }
                    }
                }
            ...
            var sln = string.Join(",", ingredientToAllergen.OrderBy(kv => kv.Value).Select(kv => kv.Key));
        ...
        private string testcase1 = @"mxmxvkd kfcds sqjhc nhms (contains dairy, fish)
trh fvjkl sbzzf mxmxvkd (contains dairy)
sqjhc fvjkl (contains soy)
sqjhc mxmxvkd sbzzf (contains fish)";

        private string input = @"rgt lmpxcr ssgkt klfp dfpqn scqc vzzxl hphcb pbmhl xrlmvz fvd cpj kxl gpplt jkrsn bhlbm dkxr vrzkz drpxzr bxhsq cvz nvhnsg jtzsqznl fjbjj slglnd qdbq gnrb zpbbvvm bqb mbdksj nlxh gtzp jrlvkz ctmzsr znjp zvmnv lhsgbq pdxhgx jrcczb vrpzss qdbj lsc jhrgz hqlm qlt kh grjf brhfcbt zjsh pkmbk rncjx nmszz fzpds qglps zjsz qfnq ftfx bbkth rkzqs vgp (contains nuts, shellfish)

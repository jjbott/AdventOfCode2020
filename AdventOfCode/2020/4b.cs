...
namespace AdventOfCode._2020
{
    ...
    {
        public static bool IsInRange(this string s, int min, int max)
        {
            int value;
        ...
        public static bool IsValidHeight(this string s)
        {
            // If cm, the number must be at least 150 and at most 193.
            // If in, the number must be at least 59 and at most 76.
            if ( s.EndsWith("cm"))
            {
                return s.Substring(0, s.Length - 2).IsInRange(150, 193);
            }  
            else if (s.EndsWith("in"))
            {
                return s.Substring(0, s.Length - 2).IsInRange(59, 76);
            }
    ...
    public class _4b
    {
        public void Run()
        {
            var data = input
                .Split("\r\n\r\n")
                .Select(x => x.Replace("\r\n", " ").Split(" ").ToDictionary(kv => kv.Split(":")[0], kv => kv.Split(":")[1]));
            /*
            ...
            */
            ...
            var count = data.Where(d =>
                d.ContainsKey("byr") && d["byr"].IsInRange(1920, 2020)
                && d.ContainsKey("iyr") && d["iyr"].IsInRange(2010, 2020)
                && d.ContainsKey("eyr") && d["eyr"].IsInRange(2020, 2030)
                && d.ContainsKey("hgt") && d["hgt"].IsValidHeight()
                && d.ContainsKey("hcl") && Regex.IsMatch(d["hcl"], "^#[0-9a-f]{6}$")
                && d.ContainsKey("ecl") && Regex.IsMatch(d["ecl"], "^(amb|blu|brn|gry|grn|hzl|oth)$")
                && d.ContainsKey("pid") && Regex.IsMatch(d["pid"], "^[0-9]{9}$")
                )
                .Count();

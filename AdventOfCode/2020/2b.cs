...
namespace AdventOfCode._2020
{
    class _2b
    {
        public void Run()
        {
            var sln = data.Split("\r\n")
                .Select(line =>
                 {
                     var data = line.Split(" ");
                     return (data[0].Split("-").Select(x => int.Parse(x)).ToArray(), data[1].Replace(":", ""), data[2]);
                 })
                 .Where(x =>
                 {
                     var pass = x.Item3;//.PadRight(30, ' ');
                     return pass[x.Item1[0]-1] == x.Item2[0] ^ pass[x.Item1[1]-1] == x.Item2[0];
                 })
                 .Count();
        ...
        private string data = @"3-4 j: tjjj
